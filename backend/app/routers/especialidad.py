...
async def obtener_especialida(id_especialidad: int, db:AsyncSession = Depends(get_db)):
    result = await db.execute(select(Especialidad).where(Especialidad.id_especialidad == id_especialidad))
    especialidad = result.scalar_one_or_none()
    if not especialidad:
        raise HTTPException(status_code=404,detail="Especialidad no encontrada")
    return especialidad

#Crear una especialdiad nueva
@router.post("/",response_model=EspecialidadOut)
async def crear_especialidad(datos:EspecialidadCreate,db: AsyncSession = Depends(get_db)):
...
#actualizar una especilidad en particular
@router.put("/{id_especialidad}",response_model=EspecialidadOut)
async def actulizar_especialidad(id_especialidad: int,datos: EspecialidadCreate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Especialidad).where(Especialidad.id_especialidad == id_especialidad))
    especialidad = result.scalar_one_or_none()
    if not especialidad:
        raise HTTPException(status_code=404,detail="Especialidad no encontrada")
